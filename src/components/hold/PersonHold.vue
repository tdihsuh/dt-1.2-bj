  ...
  <div class="box">
    <div class="box_entlist" key="3-1">
      <div class="content_search">
        <div class="ipt_box">
          <Input v-model="persontable.keyword" placeholder="输入关键词" size="large" key="3-1"> </Input>
        </div>
    ...
    <div class="content_box" style="min-width:660px;">
      <div class="content_table">
        ...
        <Table :columns="columns" :data="persontable.table" size="large" @on-selection-change="selection"
               @on-sort-change="sort" key="1"></Table>
      </div>
      <div class="table_page" >
        <Page :total="persontable.totalPage" @on-change="page" show-elevator :current="persontable.current"
              key="2"></Page>
      </div>
...
<script>
  ...
  export default {
    name: 'personhold',
    mounted: function () {
      this.$store.dispatch('HoldFetchPerson');
    },
    data: function () {
      return {
        modal: false,
        ...
        columns: [
          ...
          },
          {
            ...
            align: 'center',
            render: (h, params) => {
            let val =params.row.code?params.row.code:'';
            let arr =val.split("");
            arr[10]="*";
            arr[11]="*";
            arr[12]="*";
          ...
          },
          {
            ...
            align: 'center',
            render: (h, params) => {
              let value = params.row.mobile ? params.row.mobile.replace(/(\d{3})\d{4}(\d{4})/,'$1***$2') : '- -'
              return h('div', value)
            },
          },
          {
